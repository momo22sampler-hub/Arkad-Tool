...
import { ArrowLeft, FileText, TrendingUp, DollarSign, Activity, Calendar } from 'lucide-react';
import EducationalTooltip from '../shared/EducationalTooltip';
import AIAnalysisSection from './AIAnalysisSection';

const DataRow = ({ label, value, highlight = false, tooltip = null }) => (
  <div style={{
    ...
    <div style={{ fontWeight: 'bold', color: highlight ? '#60a5fa' : '#f1f5f9' }}>
      {value || '---'}
    </div>
  </div>
);
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: '8px', marginTop: '12px' }}>
            <span style={{
              ...
              backgroundColor: 'rgba(6, 182, 212, 0.1)', color: '#06b6d4', border: '1px solid rgba(6, 182, 212, 0.3)'
            }}>{on.tipo}</span>
            <span style={{
              fontSize: '10px', padding: '4px 12px', borderRadius: '8px', fontWeight: 'black', textTransform: 'uppercase',
              backgroundColor: '#1e293b', color: '#94a3b8', border: '1px solid #334155'
            }}>{getCurrencyLabel(on.currency)}</span>
          </div>
        </div>

        {/* 4 MÉTRICAS CLAVE */}
        <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
          <MetricCard label="TIR Estimada" value={on.tir != null ? `${on.tir}%` : '---'} color="#4ade80" tooltip="TIR" />
          <MetricCard label="Paridad" value={on.parity != null ? `${on.parity}%` : '---'} color="#60a5fa" tooltip="paridad" />
          <MetricCard label="Precio Actual" value={on.price != null ? `${getCurrencySymbol(on.currency)} ${on.price?.toFixed(2)}` : '---'} color="#fbbf24" />
          <MetricCard label="Mod. Duration" value={on.modified_duration != null ? on.modified_duration : '---'} color="#a78bfa" tooltip="modified_duration" />
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '40px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
          ...
          <section>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 48px' }}>
              <DataRow label="Precio Dirty" value={`${getCurrencySymbol(on.currency)} ${on.dirty_price?.toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) || on.price?.toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) || '---'}`} highlight tooltip="dirty_price" />
              <DataRow label="Precio Clean" value={`${getCurrencySymbol(on.currency)} ${on.clean_price?.toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) || '---'}`} tooltip="clean_price" />
              <DataRow label="Interés Corrido (IC)" value={`${getCurrencySymbol(on.currency)} ${on.accrued_interest?.toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) || '---'}`} tooltip="interes_corrido" />
              <DataRow label="Valor Técnico (VT)" value={`${getCurrencySymbol(on.currency)} ${on.technical_value?.toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) || '---'}`} highlight />
            </div>
          </section>
          ...
          {on.expiration && (
            <section>
              ...
              <div style={{ backgroundColor: '#0f172a', border: '1px solid #1e293b', padding: '16px', borderRadius: '12px' }}>
                <div style={{ fontSize: '18px', fontWeight: 'bold', color: '#f1f5f9' }}>{on.expiration}</div>
              </div>
            </section>
          )}
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
          ...
          <section style={{
            ...
            border: '1px solid rgba(6, 182, 212, 0.2)', padding: '24px', borderRadius: '16px'
          }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px' }}>
              <DollarSign size={14} style={{ color: '#06b6d4' }} />
              <h3 style={{ fontSize: '10px', fontWeight: 'black', color: '#06b6d4', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                Precio Actual
              </h3>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              {on.variation !== undefined && on.variation !== null && (
                <div style={{
                  ...
                  <div style={{ fontSize: '9px', color: on.variation >= 0 ? '#22c55e' : '#ef4444', fontWeight: 'black', textTransform: 'uppercase', marginBottom: '4px' }}>
                    Variación del día
                  </div>
                  <div style={{ fontSize: '20px', fontWeight: 'black', color: on.variation >= 0 ? '#4ade80' : '#ef4444' }}>
          ...
          <div style={{ backgroundColor: '#0f172a', border: '1px solid #1e293b', padding: '20px', borderRadius: '12px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '12px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <span style={{ fontWeight: 'bold', color: on.source === 'BYMA' ? '#22c55e' : '#fbbf24' }}>{on.source}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ color: '#64748b' }}>Timestamp</span>
                <span style={{ fontWeight: 'bold', color: '#cbd5e1' }}>{on.timestamp || '-'}</span>
              </div>
            </div>
          </div>
        </div>
